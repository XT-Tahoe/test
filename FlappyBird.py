...
class GameBackground(object):
    ...
    def __init__(self, scene):
        # 加载相同张图片资源,做交替实现地图滚动
        self.image1 = pygame.image.load("background.jpg")
        ...
        self.speed = 4
        # 柱子图
        self.pillar = pygame.image.load("pillar.png")
...
class MainScene(object):
    ...
    def __init__(self):
        ...
        self.point = 0
        # 设置标题及得分
        pygame.display.set_caption("Flappy Bird        Point:" + str(int(self.point)))
        # 暂停
        self.pause = False
        # 创建地图对象
    ...
    def draw_elements(self):
        self.map.draw()
        self.bird.draw()
        pygame.display.set_caption("Flappy Bird       得分:" + str(float('%.2f' % self.point)))
    ...
    def detect_conlision(self):
        ...
        if self.map.pillar_positions_x[0] <= self.bird.size_x + self.bird.x and self.map.pillar_positions_x[0] >= -60:
            if self.map.pillar_positions_y[0] + 400 < self.bird.y and self.bird.y < self.map.pillar_positions_y[
                0] + 600:
        ...
        while True:
            ...
            if self.pause == False and self.lose == False:
                ...
                if state == 1:
                    ...
                    pygame.display.set_caption("Flappy Bird  Game Over   Point:" + str(float('%.2f' % self.point)))
                    self.lose = True
                # 刷新显示
